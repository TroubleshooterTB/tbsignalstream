...
Φ (Expectancy) = (Win Rate × Avg Win) - (Loss Rate × Avg Loss)
"""
...
class AlphaEnsembleScreener:
    ...
    def _get_nifty_200_list(self) -> List[Dict]:
        ...
        if os.path.exists(json_file):
            try:
                ...
                logger.info(f"📊 Loaded {len(nifty_200)} Nifty 200 constituents from {json_file}")
                return nifty_200
            except Exception as e:
                logger.warning(f"⚠️ Error loading {json_file}: {e}")
        
        # Fallback: Use Nifty 50 subset for testing
        logger.warning(f"⚠️ {json_file} not found - using Nifty 50 subset")
        logger.warning("⚠️ Run 'python fetch_nifty200_symbols.py' to generate complete list")
        ...
        logger.info(f"📊 Using Nifty 50 subset: {len(nifty_50_subset)} constituents")
    ...
    def screen_stocks(self, screening_date: str, nifty_data: pd.DataFrame) -> List[Dict]:
        ...
        """
        logger.info(f"\n{'='*80}")
        logger.info(f"🔍 SCREENING NIFTY 200 AT 10:30 AM")
        logger.info(f"{'='*80}\n")
        ...
        logger.info(f"📊 Nifty 50: {nifty_move_pct:.2f}% from open → {nifty_direction}")
        
        if nifty_direction == 'NEUTRAL':
            logger.warning("⚠️ Nifty is NEUTRAL - reduced screening confidence")
        ...
        for stock in self.NIFTY_200_SYMBOLS:
            ...
            try:
                ...
                logger.info(f"✅ {symbol}: Move={stock_move_pct:.2f}%, ATR={atr_percent:.2f}%, "
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"🎯 TOP 25 CANDIDATES SELECTED (from {len(candidates)} qualified)")
        logger.info(f"{'='*80}\n")
    ...
    def run_backtest_with_screener(self, screening_date: str, initial_capital: float = 100000) -> Dict:
        ...
        4. Calculate performance metrics with expectancy (Φ)
        """
        logger.info(f"\n{'='*80}")
        logger.info(f"🚀 ALPHA-ENSEMBLE BACKTEST WITH DYNAMIC SCREENER")
        logger.info(f"{'='*80}")
        ...
        if nifty_data.empty:
            logger.error("❌ Failed to fetch Nifty data - cannot proceed with screening")
        ...
        if not top_25_candidates:
            logger.warning("⚠️ No candidates passed screening criteria")
            return {'trades': [], 'capital': initial_capital}
        
        # LAYER 3 & 4: Run Alpha-Ensemble strategy on selected stocks
        logger.info(f"\n{'='*80}")
        logger.info(f"🎯 EXECUTING ALPHA-ENSEMBLE ON TOP 25 CANDIDATES")
        logger.info(f"{'='*80}\n")
