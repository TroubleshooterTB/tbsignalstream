...
class DefiningOrderStrategy:
    ...
    def __init__(self, api_key: str, jwt_token: str):
        ...
        # FINE-TUNING PARAMETERS (v1.1 - Win Rate Optimization)
        self.VOLUME_SURGE_MULTIPLIER = 1.5  # Breakout needs 1.5x avg volume (was 2x)
        self.RSI_LOWER_BOUND = 25  # Avoid deep oversold (was 30)
        self.RSI_UPPER_BOUND = 75  # Avoid extreme overbought (was 70)
        self.VWAP_BUFFER_PERCENT = 0.2  # Price must be >0.2% from VWAP (was 1.0%)
        self.BREAKOUT_STRENGTH_PERCENT = 0.15  # Close must be >0.15% beyond DR (was 0.3%)
    ...
    def check_enhanced_confirmation(self, row: pd.Series, minute_data: pd.DataFrame, 
                                   ...
                                   dr_high: float, dr_low: float) -> Tuple[bool, str]:
        """
        Layer 3: ENHANCED Multi-Filter Confirmation (v1.1 - Win Rate Optimized)
        
        Filters (ALL must pass):
        1. VWAP Confirmation (tighter threshold)
        2. SuperTrend Direction
        3. Volume Surge (2x average)
        4. RSI Momentum (30-70 range)
        5. Breakout Strength (>0.3% beyond DR)
        ...
        if trade_direction == 'LONG':
            # Filter 1: VWAP with buffer (price must be >1% above VWAP)
            vwap_threshold = row['VWAP'] * (1 + self.VWAP_BUFFER_PERCENT / 100)
            if row['Close'] < vwap_threshold:
                return False, f"VWAP: ₹{row['Close']:.2f} < ₹{vwap_threshold:.2f} (req +{self.VWAP_BUFFER_PERCENT}%)"
            
            # Filter 2: SuperTrend must be bullish
            if row['SuperTrend_Direction'] != 1:
                return False, f"SuperTrend: Bearish ({row['SuperTrend_Direction']})"
            
            # Filter 3: Volume surge (must be 2x average)
            if row['Volume'] < recent_volume * self.VOLUME_SURGE_MULTIPLIER:
                return False, f"Volume: {row['Volume']:,.0f} < {recent_volume * self.VOLUME_SURGE_MULTIPLIER:,.0f} (req {self.VOLUME_SURGE_MULTIPLIER}x avg)"
            
            # Filter 4: RSI momentum (avoid overbought)
            if rsi > self.RSI_UPPER_BOUND:
                return False, f"RSI: {rsi:.1f} > {self.RSI_UPPER_BOUND} (overbought)"
            if rsi < self.RSI_LOWER_BOUND:
                return False, f"RSI: {rsi:.1f} < {self.RSI_LOWER_BOUND} (oversold)"
            
            # Filter 5: Breakout strength (close must be >0.3% above DR high)
            breakout_strength = ((row['Close'] - dr_high) / dr_high) * 100
            if breakout_strength < self.BREAKOUT_STRENGTH_PERCENT:
                return False, f"Weak breakout: {breakout_strength:.2f}% < {self.BREAKOUT_STRENGTH_PERCENT}%"
            
            return True, f"All filters passed (VWAP+{breakout_strength:.2f}%, Vol {row['Volume']/recent_volume:.1f}x, RSI {rsi:.0f})"
        
        elif trade_direction == 'SHORT':
            # Filter 1: VWAP with buffer (price must be >1% below VWAP)
            vwap_threshold = row['VWAP'] * (1 - self.VWAP_BUFFER_PERCENT / 100)
            if row['Close'] > vwap_threshold:
                return False, f"VWAP: ₹{row['Close']:.2f} > ₹{vwap_threshold:.2f} (req -{self.VWAP_BUFFER_PERCENT}%)"
            
            # Filter 2: SuperTrend must be bearish
            if row['SuperTrend_Direction'] != -1:
                return False, f"SuperTrend: Bullish ({row['SuperTrend_Direction']})"
            
            # Filter 3: Volume surge
            if row['Volume'] < recent_volume * self.VOLUME_SURGE_MULTIPLIER:
                return False, f"Volume: {row['Volume']:,.0f} < {recent_volume * self.VOLUME_SURGE_MULTIPLIER:,.0f}"
            
            # Filter 4: RSI momentum
            if rsi > self.RSI_UPPER_BOUND:
                return False, f"RSI: {rsi:.1f} > {self.RSI_UPPER_BOUND}"
            if rsi < self.RSI_LOWER_BOUND:
                return False, f"RSI: {rsi:.1f} < {self.RSI_LOWER_BOUND}"
            
            # Filter 5: Breakout strength
            breakout_strength = ((dr_low - row['Close']) / dr_low) * 100
            if breakout_strength < self.BREAKOUT_STRENGTH_PERCENT:
                return False, f"Weak breakout: {breakout_strength:.2f}%"
            
            return True, f"All filters passed (Vol {row['Volume']/recent_volume:.1f}x, RSI {rsi:.0f})"
        ...
        for symbol_info in symbols:
            ...
            for date in trading_dates:
                ...
                for idx in range(len(intraday_data)):
                    ...
                    # Layer 3: ENHANCED Multi-Filter Confirmation (v1.1)
                    minute_idx = minute_data.index.get_loc(timestamp)
                    passed, reason = self.check_enhanced_confirmation(
                        row, minute_data, minute_idx, trade_direction, dr['high'], dr['low']
                    )
