...
class DefiningOrderStrategy:
    ...
    def __init__(self, api_key: str, jwt_token: str):
        ...
        # v1.4 LOSING TRADE FIXES + v1.5 SMART EXCEPTIONS + v1.7 BALANCED FILTERS
        self.MINIMUM_SL_PERCENT = 0.5  # Minimum 0.5% stop loss (avoid noise triggers)
        self.ENTRY_CUTOFF_TIME = time(14, 30)  # Base cutoff: 2:30 PM (avoid weak late entries)
        self.LATE_ENTRY_EXCEPTION_TIME = time(15, 5)  # Allow until 3:05 PM if volume exceptional
        self.LATE_ENTRY_VOLUME_THRESHOLD = 2.5  # v1.7: Back to 2.5x (3x was too strict in v1.6)
        ...
        # v1.7 BALANCED PRECISION FILTERS (Target 60%+ Win Rate)
        self.RSI_OVERBOUGHT_LIMIT = 75  # Reject LONG if RSI > 75 (overbought trap) - KEPT from v1.6
...
if __name__ == "__main__":
    ...
    # Define test symbols (Nifty 50 stocks - highly liquid) - Expanded for comprehensive validation
    symbols = [
        {'symbol': 'RELIANCE-EQ', 'token': '2885'},      # Energy & Petrochemicals
        {'symbol': 'TCS-EQ', 'token': '11536'},           # IT Services
        {'symbol': 'HDFCBANK-EQ', 'token': '1333'},       # Banking
        {'symbol': 'INFY-EQ', 'token': '1594'},           # IT Services
        {'symbol': 'ICICIBANK-EQ', 'token': '4963'},      # Banking
        {'symbol': 'HINDUNILVR-EQ', 'token': '1394'},     # FMCG
        {'symbol': 'SBIN-EQ', 'token': '3045'},           # Banking
        {'symbol': 'BHARTIARTL-EQ', 'token': '10604'},    # Telecom
        {'symbol': 'ITC-EQ', 'token': '1660'},            # FMCG
        {'symbol': 'KOTAKBANK-EQ', 'token': '1922'},      # Banking
    ]
