"""
The Defining Order Breakout Strategy - PRECISION TARGET v1.8
(Data-Driven Optimization for 60%+ Win Rate)

v1.8 PRECISION TARGET (Dec 14, 2024):
Based on v1.7 expanded test (113 trades, 36.3% win rate, 7.98% return):

LOSS ANALYSIS FINDINGS:
- ITC-EQ: 14.3% win rate → EXCLUDED
- SBIN-EQ: 25% win rate → EXCLUDED  
- 84.7% of losers hit SL (stops too tight)
- 13:00-13:59 hour: 30 losers vs 8 winners → STRICTER FILTER
- Late entries (14:30+): 47% of losers

v1.8 IMPROVEMENTS:
1. ✅ EXCLUDE worst symbols (ITC, SBIN) - poor performers
2. ✅ WIDER STOPS: 0.6% minimum (from 0.5%) + 2.0x ATR (from 1.5x)
3. ✅ STRICTER 13:00 HOUR: Require 2.0x volume (from 1.4x) in lunch hour
4. ✅ HIGHER BASE VOLUME: 1.6x (from 1.4x) for better conviction
5. ✅ KEEP v1.7 filters: RSI >75 block, late entry 2.5x volume

Target: 60%+ win rate with maintained profitability
"""
...
class DefiningOrderStrategy:
    ...
    def __init__(self, api_key: str, jwt_token: str):
        ...
        # v1.8 PRECISION TARGET PARAMETERS (Data-Driven from 113-trade analysis)
        self.MINIMUM_SL_PERCENT = 0.6  # v1.8: 0.6% (from 0.5%) - 84.7% losers hit SL
        ...
        self.ATR_MULTIPLIER_FOR_SL = 2.0  # v1.8: 2.0x ATR (from 1.5x) - wider stops
        
        # v1.8 PRECISION FILTERS (Target 60%+ Win Rate)
        self.RSI_OVERBOUGHT_LIMIT = 75  # Reject LONG if RSI > 75
        self.VOLUME_MULTIPLIER = 1.6  # v1.8: 1.6x base volume (from 1.4x) - higher conviction
        self.LUNCH_HOUR_VOLUME = 2.0  # v1.8: 2.0x in 13:00 hour (30 losers vs 8 winners)
