"""
The Defining Order Breakout Strategy - BALANCED PRECISION v1.9
(Refined Balance Between Precision and Opportunity)

v1.9 BALANCED PRECISION (Dec 14, 2024):
Based on v1.8 test (86 trades, 37.21% win rate, 6.66% return):

v1.8 ANALYSIS:
- 37.21% WR: Marginal improvement (+0.93% vs v1.7)
- 6.66% return: Lower than v1.7's 7.98% (fewer trades)
- CRITICAL: Many "SL too tight: 0.XX% < 0.60%" rejections
- 0.6% minimum SL TOO WIDE → Missed good opportunities
- Symbol exclusion WORKING → ITC/SBIN correctly excluded
- Lunch hour filter WORKING → 2.0x volume effective
- 2.0x ATR may be excessive → Test middle ground

v1.9 REFINEMENTS (Balance Precision vs Opportunity):
1. ✅ REVERT SL minimum: 0.5% (from 0.6%) - allow more quality signals
2. ✅ MODERATE ATR: 1.75x (from 2.0x) - middle ground between 1.5x and 2.0x
3. ✅ KEEP symbol exclusion: ITC, SBIN excluded
4. ✅ KEEP lunch hour filter: 2.0x volume at 13:00
5. ✅ KEEP base volume: 1.6x for conviction
6. ✅ KEEP RSI>75 block and late entry 2.5x volume

Target: 50-60% win rate with better return than v1.8
"""
...
class DefiningOrderStrategy:
    ...
    def __init__(self, api_key: str, jwt_token: str):
        ...
        # v1.9 BALANCED PRECISION PARAMETERS (Refined from v1.8)
        self.MINIMUM_SL_PERCENT = 0.5  # v1.9: REVERTED to 0.5% (0.6% too wide)
        ...
        self.ATR_MULTIPLIER_FOR_SL = 1.75  # v1.9: 1.75x ATR (middle ground: 1.5x→2.0x)
        
        # v1.9 BALANCED FILTERS (Target 50-60% Win Rate)
        self.RSI_OVERBOUGHT_LIMIT = 75  # v1.7: Reject LONG if RSI > 75 (KEEP)
        self.VOLUME_MULTIPLIER = 1.6  # v1.8: 1.6x base volume (KEEP - working)
        self.LUNCH_HOUR_VOLUME = 2.0  # v1.8: 2.0x in 13:00 hour (KEEP - working)
    ...
    def check_enhanced_confirmation(self, row: pd.Series, minute_data: pd.DataFrame, 
        ...
        # v1.8: LUNCH HOUR STRICTER VOLUME (13:00-13:59 had 30 losers vs 8 winners)
        entry_hour = row.name.hour if hasattr(row.name, 'hour') else pd.to_datetime(row.name).hour
        volume_threshold = self.LUNCH_HOUR_VOLUME if entry_hour == 13 else self.VOLUME_MULTIPLIER
...
if __name__ == "__main__":
    ...
    # Define test symbols (v1.8: EXCLUDED worst performers ITC-14.3% & SBIN-25% win rates)
    symbols = [
        {'symbol': 'RELIANCE-EQ', 'token': '2885'},      # Energy (66.7% win rate)
        {'symbol': 'TCS-EQ', 'token': '11536'},           # IT Services
        {'symbol': 'HDFCBANK-EQ', 'token': '1333'},       # Banking (37.0% win rate)
        {'symbol': 'INFY-EQ', 'token': '1594'},           # IT Services (75.0% win rate - BEST!)
        {'symbol': 'ICICIBANK-EQ', 'token': '4963'},      # Banking (42.9% win rate)
        {'symbol': 'HINDUNILVR-EQ', 'token': '1394'},     # FMCG (37.5% win rate)
        # {'symbol': 'SBIN-EQ', 'token': '3045'},         # EXCLUDED: 25% win rate
        {'symbol': 'BHARTIARTL-EQ', 'token': '10604'},    # Telecom (37.5% win rate)
        # {'symbol': 'ITC-EQ', 'token': '1660'},          # EXCLUDED: 14.3% win rate
        {'symbol': 'KOTAKBANK-EQ', 'token': '1922'},      # Banking (33.3% win rate)
    ]
