"""
The Defining Order Breakout Strategy - ENHANCED PRECISION v2.0
(Data-Driven Enhancements Based on v1.9 Loss Analysis)

v2.0 ENHANCED PRECISION (Dec 14, 2024):
Based on v1.9 test (90 trades, 40.00% win rate, 16.38% return):

v1.9 LOSS ANALYSIS (54 losing trades analyzed):
- 40% WR: Great improvement over v1.7 (36.28%) and v1.8 (37.21%)
- 16.38% return: 2x better than v1.8 (6.66%) and v1.7 (7.98%)
- CRITICAL ISSUE 1: 13:00 hour = 44% of losers (24/54), only 25% WR
- CRITICAL ISSUE 2: 100% of losers are EOD closes (NO SL hits!)
- CRITICAL ISSUE 3: KOTAKBANK (33% WR, -₹277) & TCS (33% WR, -₹1,501)
- LONG trades: 36.36% WR vs SHORT: 42.11% WR (directional imbalance)
- Friday struggles: 33.33% WR (worst day of week)

v2.0 PHASE 1 ENHANCEMENTS (Quick Wins):
1. ✅ EXCLUDE KOTAKBANK & TCS: Remove 33% WR symbols (6 symbols total)
2. ✅ 13:00 HOUR TIGHTER SL: 1.5x ATR (vs 1.75x) for lunch hour
3. ✅ MAXIMUM SL CAP: 1.5% hard limit (prevent runaway losses)
4. ✅ FRIDAY STRICTER VOLUME: 2.5x volume requirement
5. ✅ KEEP all working v1.9 filters (0.5% SL, 1.75x ATR, lunch filter)

Target: 45-50% win rate, 18-22% return, ₹220-250 expectancy
Expected: Remove 6 worst losers, reduce 13:00 damage by 30-50%
"""
...
class DefiningOrderStrategy:
    ...
    def __init__(self, api_key: str, jwt_token: str):
        ...
        self.ATR_MULTIPLIER_FOR_SL = 1.75  # v1.9: 1.75x ATR (default for most hours)
        ...
        # v2.0 NEW PHASE 1 ENHANCEMENTS
        self.LUNCH_HOUR_ATR_MULTIPLIER = 1.5  # v2.0: Tighter SL for 13:00 hour (vs 1.75x)
        self.MAXIMUM_SL_PERCENT = 1.5  # v2.0: Hard cap on stop loss (prevent runaway losses)
    ...
    def check_enhanced_confirmation(self, row: pd.Series, minute_data: pd.DataFrame, 
        ...
        # v2.0: FRIDAY + LUNCH HOUR STRICTER VOLUME
        ...
        # v2.0: Friday requires 2.5x (33.33% WR in v1.9), Lunch 2.0x, else 1.6x
        if entry_day == 4:  # Friday (0=Monday, 4=Friday)
            volume_threshold = self.FRIDAY_VOLUME_MULTIPLIER  # 2.5x
        elif entry_hour == 13:  # Lunch hour
            volume_threshold = self.LUNCH_HOUR_VOLUME  # 2.0x
        else:
            volume_threshold = self.VOLUME_MULTIPLIER  # 1.6x
